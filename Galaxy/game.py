...
class GameInformation:
    def __init__(self, yellow_hits, red_hits, yellow_lifes, red_lifes):
        self.yellow_hits = yellow_hits
        self.red_hits = red_hits
        self.yellow_lifes = yellow_lifes
        self.red_lifes = red_lifes
...
class Game:
    ...
    def _handle_collision(self):
        ball = self.ball
        yellow_ship = ellow_sright_paddle = self.right_paddle
        ...
        if ball.x_vel < 0:
            if ball.y >= yellow_ship.y ll.y <= yellow_ship.y le.HEIGHT:
                if ball.x - ball.RADIUS <= yellow_ship.x le.WIDTH:

                    middle_y = yellow_ship.y le.HEIGHT / 2
    ...
    def draw(self, draw_score=True, draw_hits=False):
        ...
        for paddle in [self.yellow_ship, sght_paddle]:
    ...
    def move_paddle(self, left=True, up=True):
        ...
        """
        if left:
            if up and self.yellow_ship.y le.VEL < 0:
            if not up and self.yellow_ship.y le.HEIGHT > self.window_height:
            self.yellow_ship.mo
    ...
    def reset(self):
        ...
        self.ball.reset()
        self.yellow_ship.re        self.right_paddle.reset()
        self.right_score = 0
        self.left_hits = 0
        self.right_hits = 0